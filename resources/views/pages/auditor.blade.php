    ...
    <div x-show="isProcessing || auditComplete" x-cloak class="bg-[#08101f] border border-white/5 rounded-3xl p-8 shadow-2xl" data-aos="fade-up">
        ...
        <div x-show="finalReport" x-cloak class="mt-8 border-t border-slate-700 pt-8" data-aos="fade-in">
            <div class="flex items-center gap-3 mb-6">
                ...
                <h3 class="text-2xl font-bold text-white">Gemini AI Analysis</h3>
            </div>
            <div class="prose prose-invert prose-sky max-w-none" x-html="finalReport"></div>
        </div>
                ...
                this.pollingInterval = setInterval(async () => {
                    try {
                        ...
                        if(data.status === 'completed') {
                            ...
                            // Parse markdown to simple HTML if needed, or backend sends HTML
                            this.finalReport = data.final_report; 
                        } else if(data.status === 'failed') {
                            clearInterval(this.pollingInterval);
                            this.isProcessing = false;
                            this.statusMessage = 'Audit failed. ' + data.message;
                        }
                    } catch(e) {
                        console.error(e);
                    }
